...
#================= this version loads the features from the Soft + HW LSTM model====================#
class OffroadLoader(Dataset):
    def __init__(self, grid_size, train=True, demo=None, datadir='/home/n9321322/DeepIRL/vehicle-motion-forecasting-master/processed_data_v5', pre_train=False, tangent=False,
                 more_kinematic=None):
    ...
    def __getitem__(self, index):
        ...
        new_dims = []
        for original_length, new_length in zip(feat.shape, (6,self.grid_size,self.grid_size)):
             new_dims.append(np.linspace(0, original_length-1, new_length))
        coords = np.meshgrid(*new_dims, indexing='ij')
        feat = map_coordinates(feat, coords)
        
        past_traj=traj[:obs_traj_len,:]
        future_traj=traj[obs_traj_len:,:]
        ...
        #feat = np.full((1, self.grid_size, self.grid_size), normalized_curvature, dtype=np.float)
