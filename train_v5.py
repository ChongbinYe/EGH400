#===========================================================#
# This version uses Soft+ HW features (6,400) of the pedestrian trajectories and
# the trajectories from GC database.
# Uses mdp.offroad_grid_v5 to load this data and network.SimpleDeconv as the feature model
#===========================================================#
import offroad_grid_v5 as offroad_grid
import offroad_loader_v5 as offroad_loader
...
vis_per_steps = 60
test_per_steps = 2
resume = 'step274-loss1.5682469445221585.pth'
#resume = None
exp_name = 'processed_data_v5'
grid_size = 80
discount = 0.9
lr = 5e-4
n_epoch = 25
...
#net = SimpleFCN(input_size=7,hidden_size=32) # original 5 in data(hieght, height_var, R, G, B) + kinematic 5 (x,y,delta_x, delta_y,k-curvature)
step = 0
nll_cma = 0
nll_test = 0
...
if resume is None:
    ...
    checkpoint = torch.load(os.path.join('/home/n9321322/DeepIRL/vehicle-motion-forecasting-master/exp/', exp_name, resume))
    ...
    for _, (feat, past_traj, future_traj) in enumerate(train_loader):
        ...
        if step % test_per_steps == 0:
            ...
            if nll_test < best_test_nll:
                ...
                path = os.path.join('/home/n9321322/EGH400/exp', exp_name, 'step{}-loss{}.pth'.format(step, nll_test))
